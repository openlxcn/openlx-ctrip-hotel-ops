...
import {catalog,renderReport} from '../skills/openlx-ctrip-hotel-ops/scripts/core.mjs';
const root=path.resolve(path.dirname(fileURLToPath(import.meta.url)),'..'),skill=path.join(root,'skills/openlx-ctrip-hotel-ops'),downloads=path.join(root,'public/downloads');
fs.mkdirSync(downloads,{recursive:true});
const name=`openlx-ctrip-hotel-ops-${catalog.version}.zip`,dest=path.join(downloads,name);
...
const meta={version:catalog.version,download_url:'/downloads/'+name,sha256:sha,bytes:fs.statSync(dest).size,github:'https://github.com/openlxcn/openlx-ctrip-hotel-ops',compatibility:'macOS及Linux运行验证见status.json；Windows尚未实测',file_count:files.length};
