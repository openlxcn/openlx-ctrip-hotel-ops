...
const fixture=()=>{const dir=fs.mkdtempSync(path.join(os.tmpdir(),'ctrip-store-'));const s=openStore(dir);s.db.prepare('INSERT INTO hotels VALUES(?,?,?,?)').run('hotel1','1','测试酒店',new Date().toISOString());return {s,dir,close(){s.db.close();fs.rmSync(dir,{recursive:true,force:true});}};};
test('all prices are exact and no trial or voucher package can create order',()=>{const f=fixture();try{for(const [p,amounts]of [['STANDARD',[1990,3990,9990]],['SUPREME',[6990,12990,39999]]])for(const [i,c]of ['monthly','quarterly','annual'].entries())assert.equal(f.s.createOrder('1','hotel1',p,c,'alipay').amount_fen,amounts[i]);for(const p of ['FREE','TRIAL','COUPON'])assert.throws(()=>f.s.createOrder('1','hotel1',p,'monthly','alipay'),/INVALID_ORDER/);assert.throws(()=>f.s.createOrder('other','hotel1','STANDARD','monthly','alipay'),/HOTEL_NOT_FOUND/);}finally{f.close();}});
...
test('HTTP website isolates session, requires CSRF and exposes no test-payment bypass',async()=>{const f=fixture();let server;try{
 ...
 const acct=await fetch(base+'/api/account',{headers:{Cookie:'ctrip_session=valid'}});assert.equal(acct.status,200);
...
test('registered device refresh is revocable and concurrent writers cannot both hold a lease',()=>{
 const dir=fs.mkdtempSync(path.join(os.tmpdir(),'ctrip-device-')),s=openStore(dir);
 try{
  s.db.prepare('INSERT INTO hotels VALUES(?,?,?,?)').run('hotel1','user1','测试酒店',new Date().toISOString());
  const o=s.createOrder('user1','hotel1','STANDARD','monthly','alipay');s.fulfill(o.id,'MOCK-TX-DEVICE',1990,'alipay');
