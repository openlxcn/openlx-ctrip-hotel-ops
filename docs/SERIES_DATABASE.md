...
日期2026-09-06。共享修改负责人：携程任务01a0729f-1b82-72a0-8ebf-6035548096b6。当前：本地及Linux各33项检查通过，真实MySQL独立测试表8项检查通过；中央路由及会员页已部署。携程、美团、飞猪空流均已在中央登记并回读；真实订单同步未实测。
...
先备份wx index.js、account.html、相关展示文件和产品commerce.sqlite。检查实际SHOW TABLES／SHOW COLUMNS后，将shared/wx-api/series.cjs复制到wx的src/services，series.html/js复制到wx的openlx目录。调用导出的migrate(db.pool)仅创建两张openlx_series表，再在既有JSON解析器后挂载mountSeries(app,db,requireUserSession,__dirname)。

产品密钥只在服务器/etc/openlx中生成，中央映射和每产品文件权限600，产品EnvironmentFile只写本产品文件路径。携程src/series-sync.mjs使用服务器已有SQLite，不共享许可证私钥。服务更新后核对systemd、健康接口、两端源码哈希、中央stream/revision与匿名401；真实交易验收另计。

恢复代码时停止对应新服务调用、从备份恢复文件和环境变量后重启；保留openlx_series表及outbox供对账，不自动清空或重建stream。数据库恢复到旧快照后若修订冲突，先对比中央与本地状态，不强行推进或回退ack。
