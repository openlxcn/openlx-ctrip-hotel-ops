...
export function createSync(db,{product='openlx-ctrip-hotel-ops',origin='https://wx.openlx.cn',keyFile,fetcher=fetch}={}){
  ...
  async function flush(){
    ...
    try{
      ...
      db.exec('BEGIN IMMEDIATE');try{
        const pending=db.prepare('SELECT value FROM series_sync_meta WHERE key=?').get('pending')?.value;
        const ack=Number(db.prepare('SELECT value FROM series_sync_meta WHERE key=?').get('ack').value);
        if(pending===body&&ack<=batch.revision){
          db.prepare('UPDATE series_sync_meta SET value=? WHERE key=?').run(String(batch.revision),'ack');
          db.prepare('INSERT OR REPLACE INTO series_sync_meta VALUES(?,?)').run('registered','1');
          db.prepare('DELETE FROM series_sync_meta WHERE key=?').run('pending');
          db.prepare('DELETE FROM series_outbox WHERE seq<=?').run(batch.revision);
        }else if(ack<batch.revision)throw Error('SERIES_LOCAL_REVISION_CONFLICT');
        db.exec('COMMIT');
      }catch(e){db.exec('ROLLBACK');throw e;}
