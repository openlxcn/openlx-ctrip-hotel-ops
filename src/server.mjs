...
export function createApp(options={}){
  const origin=options.origin||process.env.PUBLIC_ORIGIN||'http://127.0.0.1:1986';
  ...
  app.use('/api',(req,res,next)=>{res.set('Cache-Control','no-store');if(['POST','PUT','PATCH','DELETE'].includes(req.method)){
    const csrf=req.headers['x-csrf-token'];if(req.headers.origin!==origin||!csrf||csrf!==req.cookies.ctrip_csrf)return res.status(403).json({success:false,error:'请求来源或会话校验失败，请刷新页面'});
  }next();});
  ...
  const session=async(req,res,next)=>{try{const r=await identity('/api/user/info',undefined,req.cookies.ctrip_session||'');if(!r.success||!r.data?.id)return res.status(401).json({success:false,error:'请先登录'});req.user=r.data;next();}catch{res.status(503).json({success:false,error:'账号服务暂不可用，请稍后重试'});}};
  app.get('/health',(_req,res)=>res.json({service:catalog.id,version:catalog.version,status:'ok'}));
  app.get('/api/session',(req,res)=>{const csrf=crypto.randomBytes(24).toString('hex');res.set('Set-Cookie',cookie('ctrip_csrf',csrf,3600));res.json({success:true,csrf,has_session:!!req.cookies.ctrip_session});});
  ...
  for(const [route,target]of Object.entries(authRoutes))app.post('/api/auth/'+route,wrap(async(req,res)=>{
    ...
    const r=await identity(target,data);if(r.data?.token){res.set('Set-Cookie',cookie('ctrip_session',r.data.token,30*86400));delete r.data.token;}
  ...
  app.get('/api/export',session,(req,res)=>{const uid=String(req.user.id);res.attachment('ctrip-account-export.json').json({hotels:store.db.prepare('SELECT * FROM hotels WHERE user_id=?').all(uid),orders:store.db.prepare('SELECT * FROM orders WHERE user_id=?').all(uid),tickets:store.db.prepare('SELECT * FROM tickets WHERE user_id=?').all(uid)});});
