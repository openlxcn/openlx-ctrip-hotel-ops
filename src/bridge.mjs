...
export function commerceBridge(root,origin){
  ...
  return {available:true,options,
    async prepay(o){
      if(o.provider==='alipay'){const result=await alipay.createPrecreatePayment(o.id,o.amount_fen/100,`OpenLX携程助手-${o.plan}`,`携程运营助手${o.cycle}`);if(!result.success)throw Error(result.error||'PAYMENT_PROVIDER_ERROR');return result.data.qr_code;}
      const r=await wechat.unifiedOrderNative(config.payment.wechatPay,{body:`OpenLX携程助手-${o.plan}`,out_trade_no:o.id,total_fee_fen:o.amount_fen,notify_url:origin+'/api/payment/wechat-notify'});return r.code_url;
    },
