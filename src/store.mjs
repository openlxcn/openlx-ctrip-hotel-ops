...
import {catalog,now} from '../skills/openlx-ctrip-hotel-ops/scripts/core.mjs';
...
export function openStore(dir){
  ...
  function active(user,hotel,time=now()){return db.prepare("SELECT * FROM entitlements WHERE user_id=? AND hotel_id=? AND status='ACTIVE' AND starts_at<=? AND expires_at>? ORDER BY CASE plan WHEN 'SUPREME' THEN 2 ELSE 1 END DESC, expires_at DESC").get(String(user),hotel,time,time)||null;}
  function createOrder(user,hotel,plan,cycle,provider){
    ...
    const id='CT'+Date.now()+crypto.randomBytes(5).toString('hex').toUpperCase();
  ...
  function ticket(user,hotel,type,request){
    if(!db.prepare('SELECT id FROM hotels WHERE id=? AND user_id=?').get(hotel,String(user)))throw Error('HOTEL_NOT_FOUND');
    const e=active(user,hotel);if(type!=='SUPPORT'&&(!e||e.plan!=='SUPREME'))throw Error('SUPREME_REQUIRED');
  ...
  return {db,active,createOrder,fulfill,license,enroll,refreshLicense,writerLease,ticket,audit,publicKey:()=>fs.readFileSync(path.join(dir,'license-public.txt'),'utf8')};
}
